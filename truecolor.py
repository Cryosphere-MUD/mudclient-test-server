def get_truecolor_text():
    """Test 24-bit truecolor (RGB) support"""
    
    content = "=== TrueColor (24-bit RGB) Test ===\n\n"
    
    # RGB spectrum test
    content += "RGB Spectrum:\n"
    for r in range(0, 256, 32):
        for g in range(0, 256, 32):
            for b in range(0, 256, 64):
                content += f"\033[38;2;{r};{g};{b}m\u2588\033[0m"
            content += " "
        content += "\n"
    content += "\n"
    ...
    for i in range(0, 256, 4):
        content += f"\033[38;2;{i};0;0m\u2588\033[0m"
    content += "\n"
    
    # Green gradient
    content += "Green Gradient:\n"
    for i in range(0, 256, 4):
        content += f"\033[38;2;0;{i};0m\u2588\033[0m"
    content += "\n"
    
    # Blue gradient
    content += "Blue Gradient:\n"
    for i in range(0, 256, 4):
        content += f"\033[38;2;0;0;{i}m\u2588\033[0m"
    content += "\n\n"
    
    # Background truecolor test
    content += "Background TrueColor:\n"
    colors = [(255,0,0), (0,255,0), (0,0,255), (255,255,0), (255,0,255), (0,255,255)]
    for r, g, b in colors:
        content += f"\033[48;2;{r};{g};{b}m\033[38;2;0;0;0m RGB({r},{g},{b}) \033[0m "
    content += "\n\n"
    ...
    content += "TrueColor Gray Gradients:\n"
    for step in [1, 2, 4, 8]:
        content += f"Step {step}: "
        ...
        for i in range(0, 256, step):
            if char_count >= 70:  # Stop at 70 chars to stay under 80
                break
            content += f"\033[38;2;{i};{i};{i}m\u2588\033[0m"
            char_count += 1
        content += "\n"
    content += "\n"
    
    # TrueColor gradient text examples
    content += "TrueColor Gradient Text:\n\n"
    ...
    for i, char in enumerate(text1):
        if char == ' ':
            ...
            if progress < 0.5:
                ...
                b = 0
            content += f"\033[38;2;{r};{g};{b}m{char}\033[0m"
    content += "\n"
...
TRUECOLOR_TEST = get_truecolor_text()
